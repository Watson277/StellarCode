...
def handle_skill_command(
    ...
    if parts == ["/skill", "reload"]:
        registry.reload()
        message = (
            f"Skills reloaded. {startup_summary(registry)}; changes apply next turn."
        )
