...
class SidecarServer:
    def __init__(
        ...
        *,
        ...
        self.task_routes: dict[str, _TaskRoute] = {}
    ...
    def handle(self, request: dict[str, Any]) -> bool:
        ...
        try:
            if method == "runtime.ping":
                ...
                self._close_workspace(request_id)
                ...
                self._start_rag_index(request_id)
                ...
                self._reload_skills(request_id)
            elif method == "browser.snapshot":
                self._browser_snapshot(request_id)
                ...
                self._browser_tabs(request_id)
            elif method == "diagnostics.snapshot":
                self._diagnostics_snapshot(request_id)
    ...
    def _open_workspace(self, request_id: str, params: dict[str, Any]) -> None:
        ...
        with self._task_lock:
            active_route_ids = set(self.task_routes)
        ...
        with self._task_lock:
            active_tasks = [
                {"task_id": task_id, "session_id": route.session_id}
    ...
    def _close_workspace(self, request_id: str) -> None:
        runtime = self._require_runtime()
    ...
    def _create_session(self, request_id: str, params: dict[str, Any]) -> None:
        ...
        self.project_active_sessions[self._runtime_project_id(runtime)] = session_id
    ...
    def _set_access_mode(self, request_id: str, params: dict[str, Any]) -> None:
        ...
        runtime.set_access_mode(mode)
        self.writer(response(request_id, result={"mode": mode}))
        self._emit_event(
            "access.mode_changed",
            {"mode": mode},
            session_id=self.active_session_id or f"workspace-{self.project_id}",
        )
    ...
    def _open_session(self, request_id: str, params: dict[str, Any]) -> None:
        ...
        self.session_projects[session_id] = runtime.project_id
        self.project_active_sessions[runtime.project_id] = session_id
        self.active_session_id = session_id
        self.active_task_id = self.session_tasks.get(session_id)
    ...
    def _reset_session(self, request_id: str, params: dict[str, Any]) -> None:
        runtime = self._require_runtime()
        session_id = str(params.get("session_id") or self.active_session_id or "")
        self._assert_session_idle(session_id)
    ...
    def _finish_task_terminal(
        ...
        try:
            ...
            if finalized_outcome != outcome:
                raise RuntimeError("pending terminal outcome changed during finalization")
        except Exception as exc:
    ...
    def _replay_events(self, request_id: str, params: dict[str, Any]) -> None:
        ...
        project_id = self.session_projects.get(session_id) or self.project_id or ""
    ...
    def _list_skills(self, request_id: str) -> None:
        runtime = self._require_runtime()
    ...
    def _reload_skills(self, request_id: str) -> None:
        self._assert_idle()
        runtime = self._require_runtime()
        self.writer(response(request_id, result=runtime.reload_skills()))

    def _browser_snapshot(self, request_id: str) -> None:
        runtime = self._require_runtime()
    ...
    def _browser_tabs(self, request_id: str) -> None:
        runtime = self._require_runtime()
        self.writer(response(request_id, result=runtime.browser_tabs()))

    def _rag_snapshot(self, request_id: str) -> None:
        runtime = self._require_runtime()
    ...
    def _start_rag_index(self, request_id: str) -> None:
        self._assert_idle()
        runtime = self._require_runtime()
    ...
    def _diagnostics_snapshot(self, request_id: str) -> None:
        runtime = self._require_runtime()
    ...
    def _cancel_task(self, request_id: str, params: dict[str, Any]) -> None:
        ...
        with self._task_lock:
            route = self.task_routes.get(task_id)
            matches = route is not None and (not session_id or route.session_id == session_id)
    ...
    def _resolve_approval(self, request_id: str, params: dict[str, Any]) -> None:
        ...
        if context is None:
            session_id = self.active_session_id or "runtime"
    ...
    def _project_has_active_tasks(self, project_id: str) -> bool:
        with self._task_lock:
            return any(
                self._runtime_project_id(route.runtime) == project_id
    ...
    def _register_task_route(
        ...
        with self._task_lock:
            existing = self.session_tasks.get(session_id)
            if existing and existing != task_id:
                raise RuntimeError("this conversation already has a running task")
            self.task_routes[task_id] = _TaskRoute(runtime, session_id)
            self.session_tasks[session_id] = task_id
            self.session_projects[session_id] = self._runtime_project_id(runtime)

    def _release_task_route(self, task_id: str) -> None:
        with self._task_lock:
            route = self.task_routes.pop(task_id, None)
            if route and self.session_tasks.get(route.session_id) == task_id:
                self.session_tasks.pop(route.session_id, None)
